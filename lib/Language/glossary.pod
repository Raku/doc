...
e.g., C<defined($instance)> is C<True>.
...
If a variable is declared to be of a certain type, but never defined,
then it will evaluate to the type object of that type.  As such, type objects
can often turn up in places where "undefined" or "null" would in other
languages:
