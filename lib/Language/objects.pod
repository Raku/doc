...
Finally, calling a method without arguments is done by putting an empty
argument list directly after it (C<()>) or by leaving it out completely.
...
Here, we are calling the method C<input-line-separator> on the C<$*IN> object,
give it no arguments and assign to the container it returned with the C<=>
operator.
