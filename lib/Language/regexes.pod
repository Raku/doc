...
C<rx:i/abc/> (case insensitive regexes), and round parentheses
...
enough. Fortunately, defining your own is fairly simple. Between C<< <[ ]> >>,
...
mix in some of the unicode categories above. You are also allowed to
...
Hence, if you parse a single line of an C<ini> file, it can be either a section or a
key-value pair and the regex would be (to a first approximation):

    / '[' \w+ ']' || \S+ \s* '=' \s* \S* /

That is, either a word surrounded by square brackets, or a string of non-whitespace
...
But sometimes this is not what you want.  For instance, you might want to match the whole
...
same level as the C<EOS> marker, so that the first, second and last lines have no
...
make the string matched within the group available as a variable:
...
To get only the grouping behavior, you can use square brackets C<[ ... ]> instead.
...
There is a more convenient way to get named captures which is discussed in the
next section.
...
Named regexes can and should be grouped in L<grammars|/language/grammars>.
...
and influence how the Perl 6 compiler translates the regex into binary code.

For example, the C<:ignorecase> (C<:i>) adverb tells the compiler to
...
otherwise fails. For instance, you don't expect the word C<motif> to be parsed as the
identifier C<mot> followed by the keyword C<if>; rather you expect C<motif> to
be parsed as one identifier and if the parser expects an C<if> afterwards,
...
to declaring a ratcheting regex:
...
shop <.ws> />. By default, C<< <.ws> >> makes sure that words are separated, so
C<a    b> and C<^&> will match C<< <.ws> >> in the middle, but C<ab> won't.

Where whitespace in a regex turns into C«<.ws>» depends on what comes before the
whitespace. In the above example, whitespace in the beginning of a regex
doesn't turn into C«<.ws>», but whitespace after characters does. In general,
the rule is that if a term might match something, whitespace after it will turn
into C«<.ws>».
...
Instead of searching for just one match and returning a L<Match object|/type/Match>, search
for every non-overlapping match and return them in a L<List|/type/List>.  In
order to do this use the C<:global> adverb:
...
As a rule of thumb, use whitespace around atoms and inside groups.
Put quantifiers directly after the atom, without inserting a blank.
Vertically align opening and closing brackets and parentheses.
...
might want to make the decimal point optional if an exponent is there.
...
there are no ambiguities possible.
...
whole file, suddenly the regex parses
...
or C<regex>) is very handy for implicitly parsing whitespace that can appear in
many places.
...
which is probably not as liberal as we want it to be. Since the user might put
...
blank between the two newlines. To allow such input strings, replace C<\n+>
by C<\n\s*>.
