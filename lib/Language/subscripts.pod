...
example, C<Match> objects support both to access different data. Also, as a
special convenience (provided by class L<Any>), any object that does not itself
implement support for Positional subscripting is treated like a list of one
element (whereas associative subscripting throws a run-time error when applied
to an object that does not implement support for it):
