...
example, C<Match> objects support both to access different sets of data. Also,
to make list munging more convenient, class L<Any> provides a fallback
implementation for positional subscripts which simply treats the left operand
as a list of one element (whereas there's no such fallback for associative
subscripts, so they throw a run-time error when applied to an object that does
...
Positional indices are counted from the start of the collection, but sometimes
you'll want to refer to elements by their position relative to the end. You can
do that using the following notation:
...
the collection:
    ...
    @array[$i % *];   # wrap around a given index ("modular arithmetic")
    @array[ -> $size { $i % $size } ];  # same as previous, with explicit block
