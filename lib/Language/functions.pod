...
Subroutines default to lexical (C<my>) scoping, and are calls to them are
...
specifiy  (or leave open) both the number and types of arguments, and also the
return value.
...
Traits are subs of the form C<< trait_mod<VERB> >>, where VERB stands for the
...
See L<type Routine|/type/Routine> for documentation of built-in routine
traits.
