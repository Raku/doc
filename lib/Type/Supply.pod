...
Returns a new supply that receives the same events as the original one, but
with each event transformed through C<&mapper>;
...
the number of events (using :elems) or the maximum number of seconds
...
Creates a new supply in which changes to the number of events seen are
...
Creates an on-demand supply from the items passed to this method.
