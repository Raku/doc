...
not a Hash:
    ...
    given (%fruits) {
      my $works = { red => %fruits<apple>, green => %fruits<pear> };
      say $works.WHAT;          # Hash
      say $works<red>;          # McIntosh
...
If you B<need> to refer to C<$_> E<0x2014> i.e., the target Hash doesn't I<have> a name
E<0x2014> you might want to consider using the more idiomatic C«my %h = a => 1, b => 2;»
form, instead:

    given ({ apple => 'McIntosh', pear => 'Bartlett' }) {
      my %yummy = red => $_<apple>, green => $_<pear>;
      say %yummy.WHAT;          # Hash
      say %yummy<red>;          # McIntosh
    }

And finally, in the spirit of TMTOWTDI, you I<could> also use
L<string interpolation|/language/quoting.html#Interpolation%3A_qq> to prevent the
compiler from interpreting the C<$_>, but that smells funny[*]...
...
* L<https://en.wikipedia.org/wiki/Code_smell|https://en.wikipedia.org/wiki/Code_smell>
