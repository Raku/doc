...
'<IN>', '<OUT>' or '<ERR>' as appropriate.  It is unlikely that you
will ever need to construct one of these objects yourself however as it
will be one for you when the special handles are initialised.
...
This returns '<IN>', '<OUT>' or '<ERR>' as approriate.
