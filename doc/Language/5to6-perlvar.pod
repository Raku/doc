...
remembering which modules have been loaded already, it's up to each
Repository to expose this knowledge if necessary.  To date, there is no
generic API for this.
