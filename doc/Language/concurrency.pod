...
C<on-demand> broadcast is like Netflix: everyone who starts streaming a movie,
always starts it from the beginning, regardless of how many people are
watching it right now. Note that no history is kept for C<on-demand> supplies,
instead, the C<supply> block is run for each tap of the supply.
...
Or alternatively as an C<on-demand> L<Supply> created by the C<supply>
keyword:
...
This could also be written using the X<C<react>> keyword:
