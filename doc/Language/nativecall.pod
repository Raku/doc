...
Example of invoking a function pointer "$fptr" returned by a function "f", using sub "def"
as definition of the desired function signature and return value:

    sub f() returns Pointer is native('mylib') { * }
    my $fptr = f();

    sub def() returns int32 is native('mylib') { * }

    my &newfunc = nativecast(&def, $fptr);
...
The C<native> trait accept a simple lib name or a full path.
