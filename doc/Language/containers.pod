...
type then the type of the variable it is bound to.

    sub lucky($type = Any) {
            my $c-value = $type.new; # closure variable, it's type defaults to Any
            return Proxy.new(
    ...
    my Int $a := lucky(Int);
    say $a = 12;
    say $a = Mu.new;  # fails with X::TypeCheck::Binding
    say $a = 13;      # fails with X::OutOfRange.new
