...
=TITLE Perl6 from Ruby - Nutshell
...
tell it is an attribute because of the C<@> sigil. In Perl 6 you must use a
C<has> declaration and a variety of sigils. You can use the C<!> twigil for
private attributes or C<.> to create an accessor. 
...
Creating a new instance of the class uses the C<.new> method. In Perl 6 you get
a default constructor that accepts key/value pairs of accessor attributes, and
can do further setup in the C<BUILD> method.
...
(so they are more like "protected" methods). In Perl 6 child objects cannot
call parent private methods.
...
meta-methods from the regular methods.
    ...
    person.class # Ruby, returns Person
    person.WHAT  # Perl 6, returns Person

    person.methods          # Ruby
    $person.^methods        # Perl 6. The .^ syntax is sugar
    $person.HOW.methods($p) # Perl 6, not using the .^ sugar

    person.instance_variables # Ruby
    person.^attributes        # Perl 6

In Perl 6, everything is an object, but not all operations are equivalent to
C<.send>.

    5.send(:+, 3)    # Ruby, returns 8
    &[+](5, 3)       # Perl 6, reference to infix additon operator
    &[+].^candidates # Perl 6, lists the signatures for the + operator
...
Finally, if a real user asks a Ruby to Perl 6 question that is not being answered here, please
add it to the document. Even if we do not have a good answer yet, that will
