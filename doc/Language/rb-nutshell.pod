...
In the future you should be able to call Ruby (and other code) directly from
your Perl 6 programs.
...
sigil can only contain things that do the C<Positional> role.
...
ways of invoking the function.
    ...
    # Ruby, declare a method and call the implicit block param
    def f
      yield 2
    end

    # Ruby, invoke f, pass it a block with 1 param
    f do |n|
      puts "Hi #{n}"
    end

    # Perl 6, declare a method with an explicit block param
    sub f(&g) {
      g(2)
    }

    # Perl 6, invoke f, pass it a block with 1 param
    # There are several other ways to do this
    f(-> $n { say "Hi {$n}" }) # Explicit param
    f -> $n { say "Hi {$n}" }  # Explicit param, no parenthesis
    f { say "Hi {$^n}" }       # Implicit param

    # Additionally, if 'f' is a method on instance 'obj' you can use C<:>
    # instead of parenthesis
    obj.f(-> $n { say "Hi {$n}" })  # Explicit param
    obj.f: -> $n { say "Hi {$n}" }  # Explicit param, no parenthesis
    obj.f: { say "Hi {$^n}" }       # Implicit param, no parenthesis
...
Perl 6 has many more advanced ways of passing arguments and receiving
parameters, see L<Signatures|/language/functions#Signatures> and
...
The C<@> sigil indicates positional slicing capabilities for a variable, and
...
Similar usage in both Ruby and Perl 6:
...
=head3 C«<< >>» Numeric shift left, right ops, shovel operator
...
mark, and two exclamation points instead of one colon.
...
Ruby uses C<.to_s> to get string context, Perl 6 usually uses C<.Str>, or
C<.gist>.
...
C<for> loops are rare in Ruby, instead we typically use C<.each> on an enumerable. The most direct translation to Perl 6 would be to use C<.map> for both C<.each> and C<.map>.
...
=head3 C<.match> method and C<=~> operator
...
There are many more powerful features of Perl 6 regexes, especially defining
entire grammars using the same syntax. See L<Regexes|/language/regexes> and
...
=head3 Basic classes, methods, attributes
...
There are currently no automated translation tools from Ruby to Perl 6.
