...
Subroutines and methods are generally declared in this manner:

  sub    foo(...) {} # where ... represent zero or more parameters
  method foo(...) {} 

but they may also may be declared with whitespace before the opening parenthesis:

  sub    foo (...) {} # where ... represent zero or more parameters
  method foo (...) {} 
 
However, such whitespace can result in errors with certain argument
lists when calling them using the function call form (see below).
...
The lesson is: "be careful with spaces following sub names."  As a
general rule, good practice might be to avoid the space after a
function name when using the function call format.
