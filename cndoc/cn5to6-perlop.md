...
和`chdir`。诸如此类函数都在[5to6-perlfunc.pod6](#language-5to6-perlfunc)，
还有圆括号依然用于分组。
...
上面提到过，请查看[5to6-perlfunc.pod6](#language-5to6-perlfunc).
...
在列表上下文，`..`作为范围运算符没有变化，另外还增加了一些很有用的范围运算符。
它们是：

    infix  ..^  不包括终点。 C<1..^5>; # 1 2 3 4 >

    infix  ^..  不包括起点。 C<1^..5>; # 2 3 4 5>

    infix  ^..^ 不包括起始。C<1^..^5>; # 2 3 4>

    prefix ^    从0开始,不包括终点。 C<^5>; # 0 1 2 3 4>

在标量上下文，`..`和`...`在Perl5中作为flip-flop运算符，但是鲜为人知和使用。 
这些运算符在Perl6中被`ff`以及`fff`取代了，但目前还没有确切的文档。
...
在[5to6-perlvar.pod](#language-5to6-perlvar)有提到过，魔法输入文件句柄`ARGV`
