...
- tr///
    ...
    也就是使用Perl的范围操作符。在 Perl6中，`tr///`有一个文档完善的对象方法版本，
    叫做`.trans`。`.trans`接受一系列的`.trans`列表对，例如
     `$x.trans(['a'..'c'] => ['A'..'C'], ['d'..'q'] =>
    ['D'..'Q'], ['r'..'z'] => ['R'..'Z']);`。关于`.trans`详细使用方法请浏览
    [https://design.perl6.org/S05.html#Transliteration](https://design.perl6.org/S05.html#Transliteration)。等价的`y///`已被去除。
