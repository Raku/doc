...
这可能用来给一个例程传递一个引用，但是在Perl 6 ，默认行为就是传递一个引用
...
请注意，这种语法中块范围不是必须的，当你不需要额外的范围时候，这非常方便
...
在编译时查找时候操作。底层逻辑和Perl 5有很大不同，但是大多数情况下C<use lib>
和Perl5中的并没有两样。
...
在Perl6，这简化成了这样：
    ...
    my @lines = "file".IO.lines; # 自动chomp
...
还有一个给开发者准备debug辅助函数`dd`（简化版的数据Dumper），它会打印`.perl`
